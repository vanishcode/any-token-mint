...
A test DApp for minting an arbitrary ERC20 token from any connected wallet on any EVM network. The user enters a network, an ERC20 contract address, an amount, and a recipient, then calls the token's `mint` function. UI follows `mint.png` at the repo root: a centered "Mint Tokens" card with Network select → Token address → Amount → Recipient → primary "Mint Tokens" button, plus a top bar with "Connect / Wrong network" on the right (RainbowKit).
...
- **"Wrong network"** state in the header (per `mint.png`) maps to wagmi's `useAccount().chain` vs. the chain selected in the form — show the switch-network action via RainbowKit's modal.
